...
const socials = [
  ...
  {
    name: "LinkedIn",
    href: "https://www.linkedin.com/in/ian-sankaka-189b8a209/",
    brandColor: "#0A66C2",
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="size-[18px]">
...
export function SocialIcons() {
  ...
  return (
    <div className="relative flex items-center gap-0.5 px-1.5 py-1.5 rounded-2xl bg-muted/80 border border-border">
      ...
      {socials.map((social, index) => {
        ...
        const color = "brandColor" in social ? social.brandColor : "#71717a";
          ...
          <a
            ...
            style={
              isHovered
                ? {
                    color,
                    boxShadow: `0 0 20px ${hexToRgba(color, 0.5)}, 0 0 40px ${hexToRgba(color, 0.25)}`,
                  }
                : undefined
            }
          ...
          >
            <span
              className={`absolute inset-1 rounded-lg transition-all duration-300 ease-out ${
                isHovered ? "opacity-100 scale-100" : "opacity-0 scale-90 bg-foreground/[0.08]"
              }`}
              style={isHovered ? { backgroundColor: hexToRgba(color, 0.15) } : undefined}
            />

            <span
              className={`relative z-10 transition-all duration-300 ease-out ${
                isHovered ? "scale-110" : "text-muted-foreground"
              }`}
            >
              {social.icon}
            </span>
            ...
            <span
              className={`absolute bottom-1.5 left-1/2 -translate-x-1/2 h-[2px] rounded-full transition-all duration-300 ease-out ${
                isHovered ? "w-3 opacity-100" : "w-0 opacity-0"
              }`}
